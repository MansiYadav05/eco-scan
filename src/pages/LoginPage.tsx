...
export const LoginPage: React.FC = () => {
  const { authMode, setAuthMode, userEmail, setUserEmail, ecoScore, history } = useApp();
  const [emailInput, setEmailInput] = useState(userEmail || 'mansiiydv158@gmail.com');
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setAuthMode('login');
    setLoginSuccessNotice('Successfully authenticated! EcoScore and history are now active.');
    setTimeout(() => {
      navigate('/classify');
    }, 1200);
  };
          ...
          <div className="bg-white rounded-3xl border border-neutral-200/90 p-6 sm:p-7 shadow-sm space-y-6">
            ...
            <div className="space-y-2.5 pt-2">
              ...
              >
                ...
                <span>Switch to Guest Mode (Zero Storage)</span>
              </button>
            </div>
          </div>
        ) : (
